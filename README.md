...
- **Windows only.** Стэк привязан к MSVC, OpenNI2 SDK и проприетарным драйверам Orbbec.
- **Нет поворотного стола / автосохранения по таймеру.** В текущей версии убраны — ICP делает мерж сканов из произвольных ракурсов без калиброванной поворотной платформы.
- **Нет GUI-диалога настроек.** Параметры редактируются через UI-группы на вкладках и сохраняются в `QSettings`. Отдельное окно «Настройки → Параметры» не реализовано.
- **Ориентация нормалей через view-point.** Для сильно замкнутых объектов может потребоваться ручная переориентация перед Poisson.

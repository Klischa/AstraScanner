...
- **Ручная очистка облака лассо-выделением.** На вкладке «Обработка» → «Ручное редактирование (лассо)»: обведите курсором контур во вьюере, нажмите «Удалить выделенное» / «Оставить только выделенное». Контур отрисовывается живьём, Esc отменяет выделение, «Отменить правку» возвращает предыдущий снимок (глубина undo — до 10 шагов).

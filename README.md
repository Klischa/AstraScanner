...
| Фильтры | SOR (Statistical Outlier Removal), ROR (Radius Outlier Removal), Voxel Grid, «Magic Wand» (Voxel + SOR). Выполняются в фоновом потоке (GUI не блокируется). |
| Undo/Redo | Отмена и повтор операций с облаком (`Ctrl+Z` / `Ctrl+Y`). Стек до 20 состояний. Меню «Правка» с динамическими названиями действий. |
...
| Тесты | Google Test + CTest (`-DASTRA_BUILD_TESTS=ON`): CloudHistory, PointCloudFilters. |
| Документация | Doxygen (`doxygen Doxyfile` → `docs/html/index.html`). |
| Горячие клавиши | `F4` Preview, `F5` Scan, `F6` Stop, `Space` Pause, `Delete` Clear, `Ctrl+Z/Y` Undo/Redo, `Ctrl+I` Import, `Ctrl+,` Settings. |
...
| `ASTRA_BUILD_TESTS` | Собрать unit-тесты (требуется GTest) | `OFF` |
...
│
├── history/
│   └── CloudHistory.h / .cpp         — Undo/Redo стек состояний облака (до 20 снимков)
│
├── settings/
...
│
└── gui/
    ├── MainWindow.h / .cpp           — главное окно, 6 вкладок, поворотный стол, меню, горячие клавиши
...
tests/
├── CMakeLists.txt                    — конфигурация тестов (GTest + CTest)
├── test_CloudHistory.cpp             — тесты Undo/Redo стека
└── test_PointCloudFilters.cpp        — тесты SOR, ROR, VoxelGrid, MagicWand
```
...
```bash
cmake -B build -S . -DASTRA_BUILD_TESTS=ON -DASTRA_ENABLE_OPENNI2=OFF
cmake --build build
cd build && ctest --output-on-failure
```

Тесты покрывают:
- **CloudHistory** — push/undo/redo, maxDepth, clear, edge cases (пустые/null облака)
- **PointCloudFilters** — SOR, ROR, VoxelGrid, MagicWand, обработка пустых/null облаков

### Проверка структуры (PowerShell)

...
static bool isUvcCameraOpened(cv::VideoCapture &capture)
{
...
{
    // Принудительно отключаем Media Foundation (MSMF), чтобы избежать ошибки WinRT
    _putenv("OPENCV_VIDEOIO_PRIORITY_MSMF=0");

    // Список предпочитаемых бэкендов

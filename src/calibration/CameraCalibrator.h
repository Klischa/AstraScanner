...
public:
    ...
    int getFrameCount() const { return static_cast<int>(m_imagePoints.size()); }
    bool isCalibrated() const { return m_calibrated; }

    // RMS-ошибка последней успешной калибровки (в пикселях). 0 если калибровка
    // ещё не выполнялась. Используется GUI для оценки качества.
    double reprojectionError() const { return m_lastRms; }

    // Публичная обёртка над сигналом statusChanged: позволяет внешним клиентам
    // (например, MainWindow) сообщать пользователю о проблемах, не эмитя чужой
    // сигнал напрямую.
    void notifyStatus(const QString &msg) { emit statusChanged(msg); }
